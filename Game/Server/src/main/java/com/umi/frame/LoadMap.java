...
public class LoadMap {
    ...
    // 加载地图组成的list数组，在两个不同的目录下都进行查找一下，都找不到才抛出异常
    private List<int[]> getDataList(String prefixDir, String filename) throws Exception {
        BufferedReader br = null;
        try {
            br = new BufferedReader(new FileReader(prefixDir + filename));
        } catch (Exception e) {
            try {
